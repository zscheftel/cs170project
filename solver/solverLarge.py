...
from Greedy import solver as solver1
from zackFunction import solver as solver2
import os, ast, sys
...
for folder in os.listdir(path):
    ...
    for filename in os.listdir(path + folder):
        ...
        if G is not None and k is not None:
            ...
            solutions = []
            scores = []
            solution = solver1(G, k, s, rG)
            solutions.append(solution)
            createOutputs(outputPath, folder, solution)
            scores.append(score_output(path + folder, outputPath + folder + ".out")[0])

            for i in range(5):
                solution = solver2(G, k, s, rG)
                solutions.append(solution)
                createOutputs(outputPath, folder, solution)
                scores.append(score_output(path + folder, outputPath + folder + ".out")[0])

            scoreSolutionList = [[scores[i], solutions[i]] for i in range(len(scores))]
            def takeFirst(lst):
                return lst[0]
            scoreSolutionList = sorted(scoreSolutionList, key = takeFirst, reverse = True)
            solution = scoreSolutionList[0][1]
            print(scoreSolutionList[0][0])
